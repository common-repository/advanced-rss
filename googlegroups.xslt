            ...
            <xsl:if test="position() &lt;= $items">
                <li>
                    ...
                    <xsl:if test="$mode != 'basic'">
                        ...
                        <xsl:text> </xsl:text>
                        <cite><xsl:value-of select="php:functionString('jp_xslt_replace','/^.*\(+|\)$/s','',author)" /></cite>
                        <xsl:if test="$mode = 'full'">
                            <div class="snip">
                                <xsl:value-of select="php:functionString('jp_xslt_snip',description,150)" disable-output-escaping="yes" />
                            </div>
                        </xsl:if>
                    </xsl:if>
                </li>
